...
pub struct MmioUart8250<const BASE: usize>;

impl<const BASE: usize> MmioUart8250<BASE> {
    ...
    pub fn init(clock: usize, baud_rate: usize) {
    ...
    pub fn write_thr(value: u8) {
        unsafe { (*Self::REGS).rw[0].write(value) }
    }
    ...
    pub fn read_dlh() -> u8 {
        unsafe { (*Self::REGS).rw[1].read() }
    }

    /// write DLH (offset + 1)
    ///
    /// set divisor latch high byte in the register
    pub fn write_dlh(value: u8) {
        unsafe { (*Self::REGS).rw[1].write(value) }
    }
    ...
    pub fn write_ier(value: u8) {
        unsafe { (*Self::REGS).rw[1].write(value) }
    }

    /// get whether low power mode (16750) is enabled (IER\[5\])
    pub fn is_low_power_mode_enabled() -> bool {
        unsafe { (*Self::REGS).rw[1].read() & 0b0010_0000 != 0 }
    }

    /// toggle low power mode (16750) (IER\[5\])
    pub fn toggle_low_power_mode() {
        unsafe { (*Self::REGS).rw[1].modify(|v| v ^ 0b0010_0000) }
    }

    /// get whether sleep mode (16750) is enabled (IER\[4\])
    pub fn is_sleep_mode_enabled() -> bool {
        unsafe { (*Self::REGS).rw[1].read() & 0b0001_0000 != 0 }
    }

    /// toggle sleep mode (16750) (IER\[4\])
    pub fn toggle_sleep_mode() {
        unsafe { (*Self::REGS).rw[1].modify(|v| v ^ 0b0001_0000) }
    }

    /// get whether modem status interrupt is enabled (IER\[3\])
    pub fn is_modem_status_interrupt_enabled() -> bool {
        unsafe { (*Self::REGS).rw[1].read() & 0b0000_1000 != 0 }
    }

    /// toggle modem status interrupt (IER\[3\])
    pub fn toggle_modem_status_interrupt() {
        unsafe { (*Self::REGS).rw[1].modify(|v| v ^ 0b0000_1000) }
    }

    /// get whether receiver line status interrupt is enabled (IER\[2\])
    pub fn is_receiver_line_status_interrupt_enabled() -> bool {
        unsafe { (*Self::REGS).rw[1].read() & 0b0000_0100 != 0 }
    }

    /// toggle receiver line status interrupt (IER\[2\])
    pub fn toggle_receiver_line_status_interrupt() {
        unsafe { (*Self::REGS).rw[1].modify(|v| v ^ 0b0000_0100) }
    }

    /// get whether transmitter holding register empty interrupt is enabled (IER\[1\])
    pub fn is_transmitter_holding_register_empty_interrupt_enabled() -> bool {
        unsafe { (*Self::REGS).rw[1].read() & 0b0000_0010 != 0 }
    }

    /// toggle transmitter holding register empty interrupt (IER\[1\])
    pub fn toggle_transmitter_holding_register_empty_interrupt() {
        unsafe { (*Self::REGS).rw[1].modify(|v| v ^ 0b0000_0010) }
    }

    /// get whether received data available is enabled (IER\[0\])
    pub fn is_received_data_available_interrupt_enabled() -> bool {
        unsafe { (*Self::REGS).rw[1].read() & 0b0000_0001 != 0 }
    }

    /// toggle received data available (IER\[0\])
    pub fn toggle_received_data_available_interrupt() {
        unsafe { (*Self::REGS).rw[1].modify(|v| v ^ 0b0000_0001) }
    }
    ...
    pub fn is_64byte_fifo_enabled() -> bool {
        unsafe { (*Self::REGS).rw[2].read() & 0b0010_0000 != 0 }
    }

    /// Read IIR\[3:1\] to get interrupt type
    pub fn read_interrupt_type() -> InterruptType {
        match unsafe { (*Self::REGS).rw[2].read() & 0b0000_1110 } {
    ...
    pub fn is_interrupt_pending() -> bool {
        unsafe { (*Self::REGS).rw[2].read() & 1 != 0 }
    }
    ...
    /// > | 0     | Enable FIFOs                |       |                                   |                         |
    pub fn write_fcr(value: u8) {
        unsafe { (*Self::REGS).rw[2].write(value) }
    }
    ...
    /// > |          | 1                        | 1                            | 8 Bits      |               |
    pub fn read_lcr() -> u8 {
        unsafe { (*Self::REGS).rw[3].read() }
    }

    /// Write LCR (offset + 3)
    ///
    /// Write Line Control Register to set DLAB and the serial data protocol
    pub fn write_lcr(value: u8) {
        unsafe { (*Self::REGS).rw[3].write(value) }
    }

    /// get whether DLAB is enabled
    pub fn is_divisor_latch_accessible() -> bool {
        unsafe { (*Self::REGS).rw[3].read() & 0b1000_0000 != 0 }
    }

    /// toggle DLAB
    pub fn toggle_divisor_latch_accessible() {
        unsafe { (*Self::REGS).rw[3].modify(|v| v ^ 0b1000_0000) }
    }

    /// get parity of used data protocol
    pub fn get_parity() -> Parity {
        match unsafe { (*Self::REGS).rw[3].read() & 0b0011_1000 } {
    ...
    pub fn set_parity(parity: Parity) {
        match parity {
            Parity::No => unsafe { (*Self::REGS).rw[3].modify(|v| (v & 0b1100_0111)) },
            Parity::Odd => unsafe {
                (*Self::REGS).rw[3].modify(|v| (v & 0b1100_0111) | 0b0000_1000)
            },
            Parity::Even => unsafe {
                (*Self::REGS).rw[3].modify(|v| (v & 0b1100_0111) | 0b0001_1000)
            },
            Parity::Mark => unsafe {
                (*Self::REGS).rw[3].modify(|v| (v & 0b1100_0111) | 0b0010_1000)
            },
            Parity::Space => unsafe { (*Self::REGS).rw[3].modify(|v| v | 0b0011_1000) },
        }
    }
    ...
    pub fn get_stop_bit() -> u8 {
        ((unsafe { (*Self::REGS).rw[3].read() & 0b100 }) >> 2) + 1
    }

    /// set stop bit, only 1 and 2 can be used as `stop_bit`
    pub fn set_stop_bit(stop_bit: u8) {
        match stop_bit {
            1 => unsafe { (*Self::REGS).rw[3].modify(|v| v & 0b1111_1011) },
            2 => unsafe { (*Self::REGS).rw[3].modify(|v| v | 0b0000_0100) },
    ...
    pub fn read_mcr() -> u8 {
        unsafe { (*Self::REGS).rw[4].read() }
    }

    /// Write MCR (offset + 4)
    ///
    /// Write Modem Control Register to control flow
    pub fn write_mcr(value: u8) {
        unsafe { (*Self::REGS).rw[4].write(value) }
    }
    ...
    /// > | 0   | Data Ready                         |
    pub fn read_lsr() -> u8 {
        unsafe { (*Self::REGS).ro[0].read() }
    }

    pub fn is_received_fifo_error() -> bool {
        unsafe { (*Self::REGS).ro[0].read() & 0b1000_0000 != 0 }
    }

    pub fn is_data_holding_registers_empty() -> bool {
        unsafe { (*Self::REGS).ro[0].read() & 0b0100_0000 != 0 }
    }

    pub fn is_transmitter_holding_register_empty() -> bool {
        unsafe { (*Self::REGS).ro[0].read() & 0b0010_0000 != 0 }
    }

    pub fn is_break_interrupt() -> bool {
        unsafe { (*Self::REGS).ro[0].read() & 0b0001_0000 != 0 }
    }

    pub fn is_framing_error() -> bool {
        unsafe { (*Self::REGS).ro[0].read() & 0b0000_1000 != 0 }
    }

    pub fn is_parity_error() -> bool {
        unsafe { (*Self::REGS).ro[0].read() & 0b0000_0100 != 0 }
    }

    pub fn is_overrun_error() -> bool {
        unsafe { (*Self::REGS).ro[0].read() & 0b0000_0010 != 0 }
    }

    pub fn is_data_ready() -> bool {
        unsafe { (*Self::REGS).ro[0].read() & 0b0000_0001 != 0 }
    }
    ...
    pub fn read_msr() -> u8 {
        unsafe { (*Self::REGS).ro[1].read() }
    }

    pub fn is_carrier_detect() -> bool {
        unsafe { (*Self::REGS).ro[1].read() & 0b1000_0000 != 0 }
    }

    pub fn is_ring_indicator() -> bool {
        unsafe { (*Self::REGS).ro[1].read() & 0b0100_0000 != 0 }
    }

    pub fn is_data_set_ready() -> bool {
        unsafe { (*Self::REGS).ro[1].read() & 0b0010_0000 != 0 }
    }

    pub fn is_clear_to_send() -> bool {
        unsafe { (*Self::REGS).ro[1].read() & 0b0001_0000 != 0 }
    }

    pub fn is_delta_data_carrier_detect() -> bool {
        unsafe { (*Self::REGS).ro[1].read() & 0b0000_1000 != 0 }
    }

    pub fn is_trailing_edge_ring_indicator() -> bool {
        unsafe { (*Self::REGS).ro[1].read() & 0b0000_0100 != 0 }
    }

    pub fn is_delta_data_set_ready() -> bool {
        unsafe { (*Self::REGS).ro[1].read() & 0b0000_0010 != 0 }
    }

    pub fn is_delta_clear_to_send() -> bool {
        unsafe { (*Self::REGS).ro[1].read() & 0b0000_0001 != 0 }
    }

    pub fn read_sr() -> u8 {
        unsafe { (*Self::REGS).scratch.read() }
    }
